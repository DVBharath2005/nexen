...
import { X, Save, Camera, User as UserIcon, Settings, Bell, Shield, Moon, Sun } from 'lucide-react';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, user, onSave, theme, toggleTheme }) => {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative bg-white dark:bg-slate-900 w-[90%] md:w-full max-w-lg rounded-[32px] overflow-hidden shadow-[0_0_50px_-12px_rgba(0,0,0,0.5)] animate-in zoom-in-95 duration-200 border-2 border-slate-200 dark:border-white/20 flex flex-col md:flex-row h-[70vh] md:h-[500px] ring-1 ring-black/5 dark:ring-white/5">
        ...
        <div className="w-full md:w-48 border-b md:border-b-0 md:border-r border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5 p-4 md:p-5 flex md:flex-col gap-2 overflow-x-auto md:overflow-x-visible">
          <h3 className="hidden md:block font-black text-slate-900 dark:text-white text-base mb-4">Settings</h3>
          <button 
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-white dark:bg-slate-900">
          <div className="p-6 border-b border-slate-200 dark:border-white/10 flex items-center justify-between">
            <h4 className="font-bold text-slate-900 dark:text-white">
              {activeTab === 'profile' ? 'Profile Information' : 'Application Settings'}
            </h4>
            ...
            {activeTab === 'profile' ? (
              <form onSubmit={handleSubmit} className="space-y-6">
                <div className="flex justify-center mb-6">
                  <div className="relative group cursor-pointer" onClick={() => fileInputRef.current?.click()}>
                    <img src={formData.avatar} className="w-24 h-24 rounded-[32px] object-cover border-4 border-slate-100 dark:border-slate-800 shadow-sm" alt="Avatar" />
                ...
                <div>
                  <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5 ml-1">Bio / Headline</label>
                  <textarea 
                    value={formData.bio} 
                ...
                <button type="submit" className="w-full bg-blue-600 text-white py-3.5 rounded-2xl font-black text-sm shadow-xl shadow-blue-900/20 hover:bg-blue-700 transition-all flex items-center justify-center gap-2">
              ...
              <div className="space-y-8">
                ...
                <div className="space-y-4">
                  <h5 className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">System</h5>
                  <div className="space-y-2">
                    {['Email Notifications', 'Push Notifications', 'Desktop Alerts'].map((item) => (
                      <div key={item} className="flex items-center justify-between p-4 hover:bg-slate-50 dark:hover:bg-white/5 rounded-xl transition-colors cursor-pointer group">
                        <span className="text-sm font-bold text-slate-700 dark:text-slate-300 group-hover:text-blue-600 dark:group-hover:text-blue-400">{item}</span>
