...
import { ArrowBigUp, TrendingUp, Users, MessageSquare, Share2, Send, Lock } from 'lucide-react';
...
const PitchCard: React.FC<PitchCardProps> = ({ pitch, onVote, onAddComment, currentUser, onUserClick }) => {
    ...
    >
      <div className="flex gap-4">
        ...
        <div className="flex flex-col items-center">
          ...
          >
            <ArrowBigUp className={`w-6 h-6 ${hasVoted ? 'fill-current' : ''}`} />
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-1 text-slate-400">
              <span className="text-[10px] font-black text-slate-300">{pitch.supportersCount || 0}</span>
              <span className="text-[10px] font-medium">community supporters</span>
            </div>

            <div className="flex items-center gap-2">
              <button 
                className="flex items-center gap-1.5 px-2.5 py-1 rounded-full border border-blue-500/30 text-blue-400 text-[8px] font-black hover:bg-blue-500/10 transition-all"
              >
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-4 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                ...
                {pitch.commentList.length === 0 && (
                  <p className="text-center text-[10px] text-slate-400 py-4">No comments yet. Be the first to give feedback!</p>
                )}
              </div>
            </div>
          </motion.div>
