...
import { ArrowLeft, MapPin, Link as LinkIcon, Calendar, Briefcase } from 'lucide-react';
  ...
  return (
    <div className="md:col-span-12 lg:col-span-6 space-y-6 pb-20 md:pb-0">
        ...
        <div className="px-6 pb-6 relative">
          ...
          <div className="flex flex-wrap gap-4 mt-4 text-sm text-slate-500 dark:text-slate-400">
            {user.role && user.company && (
              <div className="flex items-center gap-1.5">
                ...
                <span>{user.role} at {user.company}</span>
              </div>
            )}
            {user.location && (
              <div className="flex items-center gap-1.5">
          ...
          <div className="flex gap-6 mt-6 pt-6 border-t border-slate-100 dark:border-white/10">
            <div className="flex gap-1.5 items-center">
              <span className="font-bold text-slate-900 dark:text-white">{user.followingIds?.length || 0}</span>
              <span className="text-slate-500 dark:text-slate-400 text-sm">Following</span>
            </div>
            <div className="flex gap-1.5 items-center">
              <span className="font-bold text-slate-900 dark:text-white">{user.followerIds?.length || 0}</span>
      ...
      <div className="space-y-4">
        <h3 className="text-lg font-bold text-slate-900 dark:text-white px-1">Posts</h3>
        {userPosts.length > 0 ? (
          userPosts.map(post => (
            <PostCard 
              key={post.id} 
          ...
          <div className="bg-white dark:bg-white/5 p-12 text-center rounded-2xl border border-dashed border-slate-200 dark:border-white/10">
            <p className="text-sm text-slate-400 font-bold italic">No posts yet.</p>
          </div>
        )}
      </div>
    </div>
