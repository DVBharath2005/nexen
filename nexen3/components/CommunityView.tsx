...
const CommunityView: React.FC<CommunityViewProps> = ({ community, posts, currentUser, onBack, savedPostIds = [], onToggleSave, onUserClick }) => {
  return (
    <div className="md:col-span-12 lg:col-span-6 space-y-4">
        ...
        <div className="px-6 pb-6 -mt-8 relative">
          <div className="flex items-end justify-between">
            <div className="flex items-end gap-4">
              ...
              <div className="mb-2">
                <h2 className="text-xl font-black text-slate-900 dark:text-white leading-none">{community.name}</h2>
          ...
          <p className="mt-4 text-sm text-slate-600 dark:text-slate-300 leading-relaxed font-medium">
             Welcome to the official {community.name} group! This is a dedicated space for builders and founders within the ecosystem to share insights and collaborate.
          </p>
        </div>
      ...
      <div className="space-y-4 pb-20 md:pb-0">
        <h3 className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-widest ml-2 mb-2">Community Discussions</h3>
        {posts.map(post => (
          <PostCard 
            key={post.id} 
            post={post} 
        ...
        {posts.length === 0 && (
          <div className="bg-slate-50 dark:bg-white/5 p-12 text-center rounded-2xl border border-dashed border-slate-300 dark:border-white/10">
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 font-bold italic">No discussions yet in this community.</p>
          </div>
        )}
      </div>
    </div>
...
const TabItem: React.FC<{ label: string; active?: boolean }> = ({ label, active }) => (
  <button className={`px-4 py-3 text-xs font-bold transition-all relative ${active ? 'text-blue-600 dark:text-blue-400' : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
    {label}
    {active && <div className="absolute bottom-0 left-4 right-4 h-0.5 bg-blue-600 dark:bg-blue-400 rounded-full" />}
  </button>
);
