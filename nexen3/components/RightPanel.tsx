...
import { TRENDING_NEWS, SUGGESTED_FOUNDERS } from '../constants';
...
const RightPanel: React.FC<RightPanelProps> = ({ followingMap, onToggleFollow }) => {
  ...
  const displayedFounders = showAllFounders ? SUGGESTED_FOUNDERS : SUGGESTED_FOUNDERS.slice(0, 3);
  ...
  return (
    <div className="hidden lg:block lg:col-span-3 space-y-6 sticky top-24 self-start">
      ...
      <div className="bg-white dark:bg-white/10 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-white/10 p-5 shadow-sm transition-all hover:shadow-lg hover:shadow-blue-200 dark:hover:shadow-blue-900/20 duration-300">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-bold text-slate-900 dark:text-white text-sm">Tech News</h3>
          </div>
          <button 
        ...
        >
          ...
          <ExternalLink className={`w-3 h-3 transition-transform ${showAllNews ? 'rotate-180' : ''}`} />
        </button>
      </div>
      ...
      <div className="bg-white dark:bg-white/10 backdrop-blur-md rounded-2xl border border-slate-200 dark:border-white/10 p-5 shadow-sm transition-all hover:shadow-lg hover:shadow-blue-200 dark:hover:shadow-blue-900/20 duration-300">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-bold text-slate-900 dark:text-white text-sm">Suggested Founders</h3>
          <button 
            onClick={handleRefresh}
            className={`text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all ${isRefreshing ? 'rotate-180 scale-110' : ''}`}
          >
      ...
      <div className="px-4 text-[10px] text-slate-500 dark:text-slate-400 flex flex-wrap gap-x-4 gap-y-2 font-medium">
        ...
        <a href="#" onClick={(e) => { e.preventDefault(); alert('Redirecting to About'); }} className="hover:underline hover:text-blue-600 dark:hover:text-blue-400 transition-colors">About</a>
        <a href="#" onClick={(e) => { e.preventDefault(); alert('Redirecting to Accessibility'); }} className="hover:underline hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Accessibility</a>
        <a href="#" onClick={(e) => { e.preventDefault(); alert('Redirecting to Help Center'); }} className="hover:underline hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Help Center</a>
        <a href="#" onClick={(e) => { e.preventDefault(); alert('Redirecting to Privacy & Terms'); }} className="hover:underline hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Privacy & Terms</a>
        <a href="#" onClick={(e) => { e.preventDefault(); alert('Redirecting to Ad Choices'); }} className="hover:underline hover:text-blue-600 dark:hover:text-blue-400 transition-colors">Ad Choices</a>
  ...
  <div className={`flex items-center justify-between ${className}`}>
    <div className="flex items-center gap-3 overflow-hidden">
      <div className="relative group shrink-0">
        <img src={avatar} className="w-9 h-9 rounded-lg shrink-0 object-cover border border-slate-200 dark:border-white/10 shadow-sm transition-transform group-hover:scale-105" alt={name} />
        <div className="absolute inset-0 bg-blue-600/0 group-hover:bg-blue-600/10 rounded-lg transition-all" />
      </div>
      <div className="min-w-0">
        <p className="text-xs font-black text-slate-900 dark:text-white truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{name}</p>
        <p className="text-[10px] text-slate-500 dark:text-slate-400 truncate font-medium">{role}</p>
      </div>
    </div>
    <button 
      onClick={onToggle}
