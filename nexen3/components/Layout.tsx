...
import { Bell, Search, Plus, Home as HomeIcon, MessageSquare, LogOut, User as UserIcon, ChevronDown, X, Sun, Moon, Settings, Star, HelpCircle } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, onTabChange, onLogout, currentUser, posts, theme, toggleTheme, onOpenSettings, onOpenPremium, onUserClick }) => {
  ...
  const handleSearch = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Search Users (deduplicated from posts)
    const matchedUsers = new Map<string, User>();
    posts.forEach(post => {
      if (post.user.name.toLowerCase().includes(lowerQuery) || post.user.username.toLowerCase().includes(lowerQuery)) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-white relative overflow-x-hidden transition-colors duration-300">
      ...
      <header className="sticky top-0 z-50 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-white/10 shadow-lg transition-colors duration-300">
        <div className="max-w-6xl mx-auto px-4 h-16 flex items-center justify-between">
          <div className="flex items-center gap-8">
            <div className="cursor-pointer" onClick={() => onTabChange('home')}><Logo className="h-10" /></div>
              ...
              {showSearch && searchQuery && (
                <div className="absolute top-full left-0 mt-2 w-96 bg-white dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-xl border border-slate-200 dark:border-white/10 overflow-hidden z-50 max-h-[80vh] overflow-y-auto">
                  {searchResults.users.length === 0 && searchResults.posts.length === 0 ? (
                    <div className="p-4 text-center text-slate-500 dark:text-slate-400 text-sm">No results found</div>
                  ) : (
                    <>
                      {searchResults.users.length > 0 && (
                        <div className="p-2">
                          <h3 className="px-2 py-1 text-xs font-bold text-slate-500 uppercase tracking-wider">People</h3>
                      ...
                      {searchResults.users.length > 0 && searchResults.posts.length > 0 && <div className="h-px bg-slate-200 dark:bg-white/10 mx-2" />}

                      {searchResults.posts.length > 0 && (
                        <div className="p-2">
                          <h3 className="px-2 py-1 text-xs font-bold text-slate-500 uppercase tracking-wider">Posts</h3>
                          {searchResults.posts.map(post => (
                            ...
                            >
                              <div className="flex items-center gap-2 mb-1">
                                <img src={post.user.avatar} alt={post.user.name} className="w-4 h-4 rounded-full" />
                                <span className="text-xs font-bold text-slate-900 dark:text-white">{post.user.name}</span>
                              </div>
          ...
          <div className="flex items-center gap-4">
            <button onClick={toggleTheme} className="p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white rounded-full transition-colors">
            ...
            <button className="p-2 text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-slate-900 dark:hover:text-white rounded-full relative transition-colors">
              <Bell className="w-5 h-5" /><span className="absolute top-2 right-2 w-2 h-2 bg-rose-500 rounded-full border-2 border-white dark:border-slate-900"></span>
            </button>
            <div className="relative">
              <button onClick={() => setShowProfileMenu(!showProfileMenu)} className="flex items-center gap-2 p-1 hover:bg-slate-100 dark:hover:bg-white/10 rounded-xl transition-all">
                  ...
                  <div className="absolute right-0 mt-2 w-56 bg-white dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl border border-slate-200 dark:border-white/10 shadow-2xl z-10 py-2">
                    <div className="px-4 py-3 border-b border-slate-200 dark:border-white/10 mb-1 flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-1">@{currentUser.username}</p>
                      </div>
                    </div>
                    ...
                    <button onClick={onLogout} className="w-full flex items-center gap-3 px-4 py-2.5 text-xs font-bold text-rose-500 hover:bg-rose-50 dark:hover:bg-white/5 transition-colors"><LogOut className="w-4 h-4" /> Logout</button>
                  </div>
                </>
              )}
            </div>
      ...
      <main className={`flex-1 ${activeTab === 'chats' || activeTab === 'ai-assistant' ? 'max-w-7xl' : 'max-w-6xl'} mx-auto w-full px-4 py-6 md:grid md:grid-cols-12 md:gap-6 relative z-10 ${activeTab === 'chats' || activeTab === 'ai-assistant' ? 'h-[calc(100vh-64px)] overflow-hidden' : ''}`}>{children}</main>
    </div>
  );
};
